...
var len3words = len(char3words)

func BenchmarkTable3Lookup(b *testing.B) {
	for i := 0; i < b.N; i++ {
		code := char3words[i%len3words]
		c, ok := country.Table3Lookup(code)
		if !ok && c != "" {
...
func BenchmarkMap3Lookup(b *testing.B) {
	for i := 0; i < b.N; i++ {
		c, ok := country.Map3Lookup(char3words[i%len3words])
		if !ok && c != "" {
			b.Fatal("expected country code to be empty when not found")
		}
	}
...
func BenchmarkTrie3Lookup(b *testing.B) {
	for i := 0; i < b.N; i++ {
		c, ok := country.Trie3Lookup(char3words[i%len3words])
		if !ok && c != "" {
			b.Fatal("expected country code to be empty when not found")
		}
	}
